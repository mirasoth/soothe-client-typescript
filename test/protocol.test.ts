...
import {
  ...
  requestEnvelope, notificationEnvelope, subscribeEnvelope, unsubscribeEnvelope,
  connectionInitEnvelope, pingEnvelope, pongEnvelope, disconnectEnvelope,
  newLoopInputMessage, newLoopNewMessage, newLoopSubscribeMessage,
  newRequestID,
  type RequestEnvelope, type ResponseEnvelope, type NextEnvelope,
  type ErrorEnvelope, type ConnectionInitEnvelope, type ConnectionAckEnvelope,
  type StatusFrame, type NotificationEnvelope, type LoopInputParams,
...
describe('round-trip', () => {
  ...
  it('notificationEnvelope has no id (fire-and-forget)', () => {
    const env = notificationEnvelope('loop_input', { loop_id: 'L1', content: 'hi' });
  ...
  it('subscribeEnvelope correlates by id', () => {
    ...
    expect(decoded).toMatchObject({ type: 'subscribe', method: 'loop_events', id: env.id });
  });
  ...
  it('ping/pong/disconnect decode', () => {
    expect(decodeMessage(`{"proto":"1","type":"ping"}`)).toMatchObject({ type: 'ping' });
    expect(decodeMessage(`{"proto":"1","type":"pong"}`)).toMatchObject({ type: 'pong' });
    expect(decodeMessage(`{"proto":"1","type":"disconnect"}`)).toMatchObject({ type: 'disconnect' });
  });
...
describe('extractSootheLoopID', () => {
  it('from StatusFrame loop_id', () => {
    const [id, ok] = extractSootheLoopID({ proto: '1', type: 'status', loop_id: 'loop-1' });
    expect(ok).toBe(true);
